...
def predict(model_file,test_data,batch_size,test_index,save=None):
    ...
    df1= pd.DataFrame(t_labels)
    df1.to_csv(os.path.join(save, 'labels18.csv'))
    df2 = pd.DataFrame(predictions)
    df2.to_csv(os.path.join(save, 'predicts18.csv'))
    df3 = pd.DataFrame(nucle_nums)
    df3.to_csv(os.path.join(save, 'nucle_nums18.csv'))
    df4 = pd.DataFrame(rna_names)
    df4.to_csv(os.path.join(save, 'rna_names18.csv'))
